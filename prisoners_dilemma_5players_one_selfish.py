...
class Agent:
    def __init__(self, name, is_shortsighted=False):
        ...
        self.is_shortsighted = is_shortsighted  # 단기적/장기적 시야 구분
        
    def get_state(self, other_players_actions):
        if self.is_shortsighted:
            # 단기적 시야: 직전 행동만 고려
...
class PrisonersDilemma:
    ...
    def plot_results(self):
        ...
        print("\n=== 최종 통계 ===")
        for i in range(5):
            player_type = "단기적 시야" if i == 0 else "장기적 시야"
            print(f"Player {i} ({player_type}) 최종 누적 보상: {self.cumulative_rewards[i][-1]:,}원")
