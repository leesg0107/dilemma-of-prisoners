...
class Agent:
    ...
    def get_state(self, other_players_actions):
        if self.is_shortsighted:  # 항상 단기적 시야 사용
...
class PrisonersDilemma:
    def __init__(self):
        # 50명의 플레이어 중 1명만 단기적 시야를 가짐
        self.agents = [
            Agent(f"Player{i}", is_shortsighted=(i == 0))  # Player0만 단기적 시야
            for i in range(50)
        ]
        self.history = []
        self.cumulative_rewards = [[0] for _ in range(50)]
        ...
        for action in actions:
            if action == 'C':
                if betrayer_count == 0:  # 모두 협력
                    rewards.append(500000)  # 협력 보상 대폭 증가
                elif betrayer_count <= 5:  # 소수만 배신
                    rewards.append(200000 - 20000 * betrayer_count)  # 기본 보상 유지
                else:  # 다수가 배신
                    rewards.append(50000)  # 최소한의 보상 보장
            else:  # action == 'D'
                if betrayer_count == 1:  # 혼자만 배신
                    rewards.append(600000)  # 배신 이득도 증가
                elif betrayer_count <= 5:  # 소수가 배신
                    rewards.append(100000)  # 소수 배신 이득
                elif betrayer_count <= 15:  # 중간 수준 배신
    ...
    def plot_results(self):
        ...
        # 누적 보상 그래프
        # 단기적 시야 플레이어는 굵게, 나머지는 얇게
        ax1.plot(self.cumulative_rewards[0], 
                label='Player0 (단기적 시야)', 
                linewidth=3, 
                color='red')
        
        # 장기적 시야 플레이어들의 평균 선
        avg_rewards = np.mean([self.cumulative_rewards[i] for i in range(1, len(self.agents))], axis=0)
        ...
        ax2.plot(range(window_size-1, len(shortsighted_history)), moving_avg_short,
                label='단기적 시야 플레이어', linestyle='--', linewidth=2, color='red')
