...
class Agent:
    def __init__(self, name, is_shortsighted=False):
        ...
        self.shortsighted_probability = 0.2  # 20% 확률로 단기적 판단
        
    def get_state(self, other_players_actions):
        if self.is_shortsighted and np.random.random() < self.shortsighted_probability:
            # 20% 확률로 단기적 시야 사용
...
class PrisonersDilemma:
    def __init__(self):
        # 15명의 플레이어 중 1명만 단기적 시야를 가짐
        self.agents = [
            Agent(f"Player{i}", is_shortsighted=(i == 0))  # Player0만 단기적 시야
            for i in range(15)
        ]
        self.history = []
        self.cumulative_rewards = [[0] for _ in range(15)]
        ...
        for action in actions:
            if action == 'C':
                if betrayer_count == 0:  # 모두 협력
                    rewards.append(100000)  # 협력 보상 증가
                elif betrayer_count <= 3:  # 소수만 배신
                    rewards.append(-10000 * betrayer_count)  # 손실 감소
                else:  # 다수가 배신
                    rewards.append(-20000)  # 최대 손실 감소
            else:  # action == 'D'
                if betrayer_count == 1:  # 혼자만 배신
                    rewards.append(120000)  # 배신 이득 증가
                elif betrayer_count <= 3:  # 소수가 배신
                    rewards.append(30000)  # 소수 배신 이득 증가
                elif betrayer_count <= 7:  # 중간 수준 배신
                    rewards.append(0)  # 중립적
                else:  # 다수가 배신
                    rewards.append(-15000)  # 최대 손실 감소
        return rewards
    ...
    def play_game(self, episodes=1000):
        for episode in range(episodes):
            ...
            if episode % 1000 == 0:
                ...
                print(f"단기적 시야 플레이어(P0)의 최근 협력 비율: {sum(1 for a in self.agents[0].history[-100:] if a == 'C')/100:.2%}")
                print("---")
    ...
    def plot_results(self):
        ...
        # 누적 보상 그래프
        for i in range(len(self.agents)):
            label = f"Player{i} ({'단기적 시야' if i == 0 else '장기적 시야'})"
            ax1.plot(self.cumulative_rewards[i], label=label, 
        ...
        # 단기적 시야 플레이어의 협력 비율
        shortsighted_history = [1 if h[0][0] == 'C' else 0 for h in self.history]
        moving_avg_short = [sum(shortsighted_history[i:i+window_size])/window_size 
                          for i in range(len(shortsighted_history)-window_size+1)]
        ax2.plot(range(window_size-1, len(shortsighted_history)), moving_avg_short,
                label='단기적 시야 플레이어', linestyle='--', linewidth=2)
        ...
        print("\n=== 최종 통계 ===")
        print("단기적 시야 플레이어:")
        print(f"Player 0: {self.cumulative_rewards[0][-1]:,}원")
        
        print("\n장기적 시야 플레이어들의 평균:")
        longterm_rewards = [self.cumulative_rewards[i][-1] for i in range(1, len(self.agents))]
        ...
        print(f"마지막 1000회 단기적 시야 플레이어의 협력 비율: {sum(1 for h in self.history[-1000:] if h[0][0] == 'C')/1000:.2%}")
