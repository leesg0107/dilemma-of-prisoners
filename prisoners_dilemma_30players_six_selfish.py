...
class PrisonersDilemma:
    def __init__(self):
        # 30명의 플레이어 중 6명이 단기적 시야를 가짐
        self.agents = [
            Agent(f"Player{i}", is_shortsighted=(i < 6))  # Player0~5가 단기적 시야
            for i in range(30)
        ]
        self.history = []
    ...
    def plot_results(self):
        ...
        # 장기적 시야 플레이어들의 평균 선
        avg_rewards = np.mean([self.cumulative_rewards[i] for i in range(6, len(self.agents))], axis=0)
        ...
        # 나머지 플레이어들은 투명하게
        for i in range(6, len(self.agents)):
            ax1.plot(self.cumulative_rewards[i], 
                    alpha=0.1, 
                    color='gray',
                    linewidth=1)
        ...
        print("\n장기적 시야 플레이어들의 평균:")
        longterm_rewards = [self.cumulative_rewards[i][-1] for i in range(6, len(self.agents))]
