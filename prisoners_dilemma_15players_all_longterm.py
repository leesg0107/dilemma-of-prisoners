...
class PrisonersDilemma:
    def __init__(self):
        # 모든 플레이어가 장기적 시야를 가짐
        self.agents = [Agent(f"Player{i}") for i in range(15)]
        self.history = []
    ...
    def plot_results(self):
        ...
        # 최종 통계 출력
        print("\n=== 최종 통계 ===")
        rewards = [self.cumulative_rewards[i][-1] for i in range(len(self.agents))]
        print(f"평균 누적 보상: {np.mean(rewards):,.0f}원")
        print(f"최대 누적 보상: {np.max(rewards):,}원")
        print(f"최소 누적 보상: {np.min(rewards):,}원")
        print(f"표준 편차: {np.std(rewards):,.0f}원")
